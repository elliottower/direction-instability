...
\begin{abstract}
A drug tested in many cell lines sometimes produces the same
transcriptomic shift everywhere and sometimes a different one in
each context. We introduce \emph{direction instability} to
distinguish these cases: the mean pairwise cosine distance among a
drug's unit-normalized gene-expression signatures across cell lines.
Low values indicate a conserved mechanism; high values indicate
context-dependent activity. The metric requires no pathway annotations
or matched genetic perturbations.
We evaluate direction instability on 8{,}949 compounds from the
LINCS L1000 Connectivity Map (978 landmark genes, 167{,}266 signatures).
Across the full dataset, instability anticorrelates with gene-level
mechanism overlap ($\rho = -0.79$). Within the HDAC inhibitor family,
the metric cleanly separates pan-isoform inhibitors, which engage a
shared chromatin remodeling program (mean instability 0.62, $n=8$),
from isoform-selective inhibitors whose activity depends on
cell-type-specific target expression (mean instability 0.96, $n=5$).
In leave-one-out cross-validation, instability computed on $N{-}1$
cell lines predicts whether the held-out line will show a consistent
drug response (AUROC $= 0.986$; permutation null $0.500 \pm 0.012$).
Direction instability is therefore a practical pre-screening filter:
before testing a compound in a new cell type, the metric identifies
whether its effect is likely to generalize.
The computation requires only a matrix of response vectors and a
context index, and applies wherever the same system is profiled
across multiple conditions.
\end{abstract}

\section{Introduction}

When a learned representation is observed across multiple contexts, a
basic geometric question arises: does the representation's direction
rotate, or does it merely rescale? The neural bracket norm
\citep{tower2026bracket} formalizes this question for neural population
vectors across evidence levels, identifying brain regions where
directional rotation tracks causal importance. We generalize this
geometric invariant into \textbf{direction instability}: the mean
pairwise angular deviation of unit-normalized representation vectors
across contexts. The metric applies whenever a system produces
high-dimensional response vectors in multiple settings and the question
is whether those responses share a conserved mechanism or are
context-dependent.

We validate direction instability on a large-scale pharmacological
testbed: the LINCS L1000 Connectivity Map, which measures 978 landmark
gene expression changes for ${\sim}20{,}000$ compounds across up to 70
cell lines \citep{subramanian2017next,lamb2006connectivity}. Each drug
in each cell line produces a high-dimensional perturbation signature.
The question---when does a drug's transcriptomic effect \emph{transport}
from one cell type to another?---is a natural instance of cross-context
representation transport. Two drugs may share a mechanism-of-action
label yet differ in how consistently they perturb gene expression across
contexts. A pan-HDAC inhibitor like vorinostat affects chromatin
remodeling in every cell, while an HDAC8-selective inhibitor like
PCI-34051 only produces a strong transcriptomic signature where HDAC8
is functionally relevant.

Direction instability decomposes the perturbation effect into two
independent components. The \emph{direction} captures which genes are
perturbed and in what relative proportions. The \emph{magnitude} captures
the overall effect size. A drug with low direction instability but high
magnitude variation has a conserved qualitative mechanism with
context-dependent penetrance.

We apply this framework to 8{,}949 drugs with $\geq5$ cell lines in
LINCS L1000. Because a drug's direction instability depends on its
cell-line count, we introduce a coverage-conditioned population null
that compares each drug against references with similar count, providing
a reusable significance test applicable to any multi-context profiling
dataset. The scorer and validation labels were frozen under a
pre-registered commit SHA (\texttt{1dc20a2}) before any real data were
analyzed.
...
achieved AUROC $= 0.986$. A note on geometric circularity: both the
predictor ($D$, built from pairwise cosines) and the outcome (held-out
cosine with consensus) derive from cosine geometry, so some predictive
relationship is expected on structural grounds alone. We therefore ran a
permutation control (Section~\ref{sec:robustness}), shuffling
drug-instability assignments 1{,}000 times. Permuted AUROC collapsed to
$0.500 \pm 0.012$ and no permutation reached the observed value
(empirical $p < 0.001$), confirming that the prediction depends on
drug-level structure, not the shared cosine construction. The separation between the most stable
...
correlation to prediction (AUROC $= 0.986$, permutation collapse to
$0.500$), with a replicate noise floor showing that cross-cell
variation exceeds assay noise for the majority of drugs.
...
triangulation provides suggestive convergent evidence from an
...
signatures more similar to genetic knockdown of their annotated
targets, though the pre-registered global test fell short of its
effect-size threshold ($\rho = -0.18$; bar $|\rho| > 0.3$).
Independent replication of the within-family result would strengthen
the case that transport is target-mediated. Two other pre-registered
predictions also missed: component-tissue enrichment showed no signal
...
neural population vectors across evidence levels. Both metrics detect
when a system responds to a perturbation by rotating its internal
state rather than scaling it. The pharmacological application replaces
evidence quartiles with cell lines and neural activations with gene
expression signatures, preserving the geometric interpretation: low
rotation implies a conserved mechanism. The analogy is structural, not
literal: evidence quartiles are ordered while cell lines are
exchangeable, so bracket norm's monotonicity properties do not carry
over. Direction instability treats all pairwise comparisons
symmetrically, which is appropriate for the unordered cell-line
setting.
...
(JUMP-CP, Perturb-seq). The HDAC selectivity gradient would be
...
LINCS L1000 measures only 978 landmark genes; full-transcriptome
profiling might reveal additional structure. The population null
controls for cell-line count but not for other confounds (drug
concentration, time point). Direction instability partially correlates
with signature $L_2$ norm ($\rho = -0.41$) and with replicate
instability ($\rho = 0.51$): drugs with stronger, more reproducible
signals tend toward lower cross-cell instability. Cross-cell $D$
exceeds the replicate noise floor for 68\% of drugs and instability
explains $1.7\times$ more held-out variance than norm alone, so the
metric captures structure beyond assay noise, but the two are
partially confounded. A Level~4 (pre-replicate-collapse) analysis
would provide a tighter noise bound than the plate-level replicates
used here. MOA annotations from the Repurposing Hub are incomplete
(20\% coverage) and may contain errors. The metric treats all cell
lines as exchangeable and does not model phylogenetic relationships
between cell types.

Whether cell-line transport predicts clinical efficacy across patient
populations remains an open question outside the scope of this work.
...
profiling datasets (JUMP-CP, Perturb-seq).
...
expression do not. Leave-one-out cross-validation confirms the metric
predicts out-of-sample consistency (AUROC $= 0.986$), making it a
practical filter for prioritizing compounds whose effects are likely
...
\citep{subramanian2017next} and the Drug Repurposing Hub
\citep{corsello2017drug}. Analysis code, pre-registered hypotheses
and effect-size thresholds (commit SHA \texttt{1dc20a2}), and
per-drug instability scores for all 8{,}949 compounds will be
