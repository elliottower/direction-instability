...
cross-context generalizability. Genetic triangulation using 154{,}993
shRNA knockdown signatures confirms the mechanism: within individual
target families, drugs with lower instability produce signatures more
similar to genetic knockdown of their annotated targets (per-family
$|\rho| = 0.32$--$0.84$, $n = 5$ families at $p < 0.05$).
\end{abstract}
...
Among 2{,}138 drug--target pairs with $\geq3$ shared cell lines, the global
correlation between direction instability and $|$drug--shRNA cosine$|$ was
modest (Spearman $\rho = -0.18$, $p = 1.1 \times 10^{-16}$). This pooled
analysis, however, conflates drug families with inherently different
baseline cosine magnitudes: a proteasome inhibitor's signature naturally
overlaps more with proteasome-subunit knockdown ($\cos \approx 0.45$) than
an HDAC inhibitor's overlaps with individual HDAC knockdown
($\cos \approx 0.15$), irrespective of instability.

Stratifying by MOA family revealed that within-family correlations are
substantially stronger (Table~\ref{tab:triangulation}). Five families
exceeded the pre-registered criterion ($|\rho| > 0.3$, $p < 0.05$),
spanning kinase inhibitors, chromatin modifiers, and receptor agonists.
The expected sign held in all five: lower instability predicted higher
drug--target cosine. Per-target-gene analysis confirmed the pattern:
KDR (VEGFR, $\rho = -0.84$, $p = 5.1 \times 10^{-7}$), CDK1
($\rho = -0.79$, $p = 0.002$), AKT1 ($\rho = -0.75$, $p = 0.002$),
and HDAC6 ($\rho = -0.71$, $p = 0.009$) each showed strong
within-target effects.

\begin{table}[t]
\centering
\caption{Genetic triangulation by MOA family: Spearman correlation between
direction instability and $|$drug--shRNA cosine$|$ for the drug's annotated
target. Negative $\rho$ indicates that mechanism-conserving drugs (low $D$)
produce signatures more similar to genetic knockdown of their target.
...
transcriptomic mechanism generalizes across cell types. Five findings
...
Genetic triangulation then confirms the mechanism from an independent
data modality: within five drug families, the signature of a
mechanism-conserving drug recapitulates the transcriptomic effect of
genetically knocking down its annotated target.
Transport is target-mediated.
